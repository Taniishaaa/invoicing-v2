...
export default function HRBatches() {
  ...
  user?.subRole === 'hr_maker'
    ? 'maker'
    : user?.subRole === 'hr_checker'
    ? 'checker'
    : user?.subRole === 'hr_approver'
    ? 'approver'
    : null
)
...
if (end)
  ...
  try {
    const r = await api.get(`/api/hr/invoices/${inv.id}/log`)
    setLog(r.data.log ?? [])
  } catch {
    setLog([])
  }
...
async function handleAction() {
  ...
  try {
    await api.patch(
      `/api/hr/invoices/${modal.invoiceId}/${modal.type}`,
      { remarks }
    )

    toast.success(`Invoice ${modal.type}d`)
...
async function handleBulkAction(action) {
  ...
  `/api/hr/batches/${detail.id}/bulk/${action}`,
  {}
)
...
async function downloadBatchExcel(batchId) {
  ...
  detail?.invoices?.filter(
    inv =>
      ...
      inv.status?.currentStage === 'extracted'
  ).length ?? 0
...
const pendingCheckerCount =
  detail?.invoices?.filter(
    inv =>
      inv.extractionStatus === 'completed' &&
      inv.status?.currentStage === 'hr_maker_verified'
  ).length ?? 0
...
const pendingApproverCount =
  detail?.invoices?.filter(
    inv =>
      inv.extractionStatus === 'completed' &&
      inv.status?.currentStage === 'hr_checker_reviewed'
  ).length ?? 0
      ...
      {detail && (
        ...
        >
          <div
            ...
            style={{ width: 800, maxWidth: '98vw', height: '100vh', background: '#fff', padding: 24, overflowY: 'auto', boxShadow: '-4px 0 20px rgba(0,0,0,0.12)' }}
          >
...
{mode === 'checker' && pendingCheckerCount > 0 && (
  ...
  >
    {bulkSaving ? 'Processing...' : '✓ Review All'}
  </button>
)}
{(mode === 'approver' ||
 user?.subRole === 'hr_approver') && pendingApproverCount > 0 && (
  <button
    className="btn btn-success btn-sm"
    disabled={bulkSaving}
    onClick={() => handleBulkAction('approve')}
  >
    {bulkSaving ? 'Processing...' : '✓ Approve All'}
  </button>
)}
                    ...
                    <tbody>
                      {(detail.invoices ?? []).map(inv => (
                        ...
                        <tr
                          key={inv.id}
                          ref={
                            applied.invoiceNumber && inv.invoiceNumber &&
                          ...
                          style={{
                            ...
                            ...(applied.invoiceNumber && inv.invoiceNumber &&
                              inv.invoiceNumber.toLowerCase().includes(applied.invoiceNumber.trim().toLowerCase())
                              ? { background: '#fff3cd', boxShadow: 'inset 0 0 0 2px #f5b301' } : {})
                          }}
                        >
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.4)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000 }}>
          <div className="card" style={{ width: 420, margin: 0 }}>
            <h3 style={{ marginTop: 0, marginBottom: 14 }}>
              {modal.type === 'verify' ? '✓ Verify Invoice' : modal.type === 'review' ? '✓ Review Invoice' : modal.type === 'approve' ? '✓ Approve Invoice' : '✕ Reject Invoice'}
            </h3>
            ...
            <div style={{ display: 'flex', gap: 8 }}>
              <button className={`btn ${modal.type === 'reject' ? 'btn-danger' : 'btn-success'}`} onClick={handleAction} disabled={saving}>
                {saving ? 'Saving...' : modal.type === 'verify' ? 'Verify' : modal.type === 'review' ? 'Review' : modal.type === 'approve' ? 'Approve' : 'Reject'}
              </button>
