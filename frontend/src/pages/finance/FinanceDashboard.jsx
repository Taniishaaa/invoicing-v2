...
const DASHBOARD_STATUS = {
  pending_clearance: 'badge badge-processing',
  cleared: 'badge badge-approved',
  paid: 'badge badge-approved',
  partially_cleared: 'badge badge-pending_review'
}
...
async function openLog(inv) {
  ...
  try {
    const r =
      await api.get(
        `/api/finance/invoices/${inv.id}/log`
      )
...
async function downloadBatchExcel(batchId) {
  ...
  <div>
    ...
    <p>Clearance and payment overview</p>
  </div>
  ...
  {
    label: 'Cleared',
    value: stats.clearedBatches,
    color: '#28a745'
  },
  {
    label: 'Paid',
    value: stats.paidBatches,
    color: '#0f766e'
  }
].map(s => (
