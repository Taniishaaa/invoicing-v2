...
const DASHBOARD_STATUS = {
  pending_verification: 'badge badge-processing',
  partially_verified: 'badge badge-pending_review',
  verified: 'badge badge-approved',
...
async function openLog(inv) {
  ...
  try {
    ...
    const r =
      await api.get(
        `/api/compliance/invoices/${inv.id}/log`
      )
...
async function downloadBatchExcel(batchId) {
  ...
  <div>
    ...
    <p>Invoice compliance verification overview</p>
  </div>
  ...
  <ExportExcel />
      ...
      >
        ...
        {[
  {
    label: 'Compliance Batches',
    value: stats.complianceBatches,
    color: '#0d6efd'
  },
  {
    label: 'Pending Compliance',
    value: stats.pendingComplianceBatches,
    color: '#856404'
  },
  {
...
>
  {batchRef(batch)}
        ...
        <td>
          ...
          <span
           className={DASHBOARD_STATUS[batch.status]}
          >
...
{
(detail.invoices ?? [])
.filter(i => i.status?.requiresCompliance)
.length
}
)
          </h4>
          ...
          <table style={{ width: '100%' }}>
            ...
            <tbody>
              {(detail.invoices ?? [])
.filter(inv => inv.status?.requiresCompliance).map(inv => (
                <tr key={inv.id}>
