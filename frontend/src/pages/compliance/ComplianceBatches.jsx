...
const STAGE_LABEL = {
    ...
    compliance_verified: 'Finance Review',
    finance_maker_verified: 'Finance Checker',
...
if (end)
  params.dateTo = end.toISOString().split("T")[0]
    ...
    const r = await api.get('/api/compliance/invoices/worklist', { params })
  ...
  try {
    const r = await api.get(`/api/compliance/invoices/${inv.id}/log`)
    setLog(r.data.log ?? [])
  } catch {
    setLog([])
  }
...
async function handleAction() {
  ...
  try {
    await api.patch(
      `/api/compliance/invoices/${modal.invoiceId}/${modal.type}`,
      { remarks }
    )
...
async function handleBulkAction() {
  ...
  `/api/compliance/batches/${detail.id}/bulk/verify`,
  {}
)
        ...
        {bulkSaving ? 'Processing...' : '✓ Verify All'}
            ...
            {detailLoading ? (
              ...
              <>
                ...
                <h4 style={{ marginBottom: 10, marginTop: 24 }}>Invoices (
{detail.invoices?.filter(
    i => i.status?.requiresCompliance
                ...
                <div style={{ overflowX: 'auto' }}>
                  <table style={{ width:'100%', borderCollapse:'collapse', fontSize:12 }}>
                    ...
                    <tbody>
                      {(detail.invoices ?? []).filter(inv => inv.status?.requiresCompliance).map(inv => (
                        <>
                        <tr
                          key={inv.id}
                          ref={
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.4)', display: 'flex', alignItems: 'flex-start', justifyContent: 'flex-end', zIndex: 1000 }}>
          <div style={{ width: 400, height: '100vh', background: '#fff', padding: 20, overflowY: 'auto', boxShadow: '-4px 0 20px rgba(0,0,0,0.1)' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
              <h3 style={{ margin: 0 }}>Compliance Activity Log</h3>
            ...
            {log.length === 0 && <p style={{ color: '#999', fontSize: 13 }}>No Compliance activity yet</p>}
            {log.map(entry => (
